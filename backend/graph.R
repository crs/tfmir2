...
get3NodeMotifs.FFL=function (tfmir.pairs.FFL,net,r)
{ 
  ...
  if(length(tfmir.pairs.FFL) > 0){
    m = c()
    motifs.FFL.primary = list(m,m,m,m)
    number = 1
      motifs = c()
      if(dim(tfmir.pairs.FFL)[1] > 0){
        tfmir.pairs = tfmir.pairs.FFL
        index=c()
        for (i  in 1 : dim(tfmir.pairs)[1]){
   ...
   print(motifs.FFL.primary)
    ...
    if(Flag==1)
    {  
      tmpres.FFL.random = matrix(ncol= no.motifs,nrow=100,rep(0,400))
      regulators=unique(as.character(unlist(net$regulator[which(net$category %in% "tf-gene" | net$category %in% "mirna-gene" | net$category %in% "mirna-mirna")])))
      ...
      for( i in 1 : as.integer(config$NO_OF_Times_FOR_Network_Randomizations_4Node))
      {
        ...
        ##randomize_conserve()
        tfmir.pairs.random.FFL=getPutativeTFmiRPairs(net.random) 
          #begin tf-miRNA
          if(dim(tfmir.pairs.random.FFL)[1] > 0){
            #print(dim(tfmir.pairs.random.FFL)[1])
            tfmir.pairs.random = tfmir.pairs.random.FFL
      ...
      { 
        ...
        if(length(motifs) != 0){ 
          ...
          MotifTypes = switch(z, "COMPOSITE-FFL","TF-3Node-FFL","miRNA-FFL", "Coreg-FFL")
