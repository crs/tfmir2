...
vennPlot <- function(counts=counts, mymain="Venn Diagram", mysub="default", setlabels="default", yoffset=seq(0,10,by=0.34), ccol=rep(1,31), colmode=1, lcol=c("#FF0000", "#008B00", "#0000FF", "#FF00FF", "#CD8500"), lines=c("#FF0000", "#008B00", "#0000FF", "#FF00FF", "#CD8500"), mylwd=3, diacol=1, type="ellipse", ccex=1.0, lcex=1.0, sepsplit="_", ...) {
  ...
  if(length(counts[[1]])==3) {
    ...
    ## Plot venn shapes
#     symbols(x=c(4, 6), y = c(6, 6), circles=c(2, 2), xlim=c(0, 10), ylim=c(0, 10), inches=F, 
#             main=mymain, sub=mysub, lwd=mylwd, xlab="", ylab="",  xaxt="n", yaxt="n", bty="n", fg=lines, ...); 
    # ony for the TFmir application
    symbols(x=c(2, 4), y = c(2, 2), circles=c(2, 2), xlim=c(0.1, 6), ylim=c(0, 4), inches=F,
            main="", sub=mysub, lwd=mylwd, xlab="", ylab="",  xaxt="n", yaxt="n", bty="n", fg=lines, ...);
    
  ## Add counts
    for(i in seq(along=counts)) {
      # for the sake of the TFmir application only 
      #olDF <- data.frame(x=c(3.1, 7.0, 5.0), y=c(6, 6, 6), counts=counts[[i]])
      olDF <- data.frame(x=c(1, 5, 3), y=c(2, 2, 2), counts=counts[[i]])
    ...
    ## comment the text only for the  application of TFmir  
...
olBarplot <- function(OLlist=OLlist, mycol="default", margins=c(6, 10, 3, 2), mincount=0, mysub="default", ...) {
  ...
  barplot(counts, col=mycol, sub=mysub, main="",...)
  par(mar=c(5, 4, 4, 2) + 0.1) # Set margins back to default
}


plotColorVenn=function(mylist,output.path,width,hight)
{
  OLlist2 <- overLapper(setlist=mylist, sep="_", type="vennsets")
  #OLlist2$Venn_List
  counts <- sapply(OLlist2$Venn_List, length);
  CairoPNG(output.path,width=width,height=hight)
  vennPlot(counts=counts,mymain="") 
  dev.off() 
}


plotColorVennBar=function(mylist,output.path,width,hight)
{
  OLlist2 <- overLapper(setlist=mylist, sep="_", type="vennsets")
  CairoPNG(output.path,bg = "transparent",width=width,height=hight,units="px")
  olBarplot(OLlist=OLlist2,mycol=c("red","green","blue"), horiz=T, las=1, cex.names=0.8) 
  dev.off()
}
