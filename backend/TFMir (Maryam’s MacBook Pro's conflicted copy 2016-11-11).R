...
columns=Reduce(intersect,input)
all.res.motifs=rbind(tf.mirna.res[,columns],mirna.mirna.res[,columns],mirna.gene.res[,columns],tf.gene.res[,columns],gene.gene.res[,columns])

names(mirnas.df)=c("node","regulation")
names(tfs.df)=c("node","regulation")
nodes.input=rbind(mirnas.df,tfs.df)
names(nodes.input)=c("target","target.reg")
all.res.motifs=merge(all.res.motifs,nodes.input,by="target")
names(nodes.input)=c("regulator","regulator.reg")
all.res.motifs=merge(all.res.motifs,nodes.input,by="regulator")

all.res.motifs.disease=all.res.motifs[all.res.motifs$is_regulator_in_disease==TRUE | all.res.motifs$is_target_in_disease==TRUE,]
all.res.motifs.tissue=all.res.motifs[all.res.motifs$is_regulator_in_tissue==TRUE | all.res.motifs$is_target_in_tissue==TRUE,]
all.res.motifs.process=all.res.motifs[all.res.motifs$is_regulator_in_process==TRUE | all.res.motifs$is_target_in_process==TRUE,]
all.res.motifs.disease.process=all.res.motifs[all.res.motifs$is_regulator_in_disease.process==TRUE | all.res.motifs$is_target_in_disease.process==TRUE,]

if(dim(all.res.motifs)[1] > 0)
  exportNetworkProperties (all.res.motifs,file.path(output.path,"all.4node"), disease,pval.cutoff,all.res.tissue.specific)

if(dim(all.res.motifs.disease)[1] > 0)
  exportNetworkProperties (all.res.motifs.disease,file.path(output.path,"disease.4node"),disease,pval.cutoff,all.res.disease.tissue.specific)

if(dim(all.res.motifs.tissue)[1] > 0)
  exportNetworkProperties (all.res.motifs.tissue,file.path(output.path,"tissue.4node"),disease,pval.cutoff,all.res.normal.tissue.specific)

if(dim(all.res.motifs.process)[1] > 0)
  exportNetworkProperties (all.res.motifs.process,file.path(output.path,"process.4node"),disease,pval.cutoff,all.res.process.tissue.specific)

if(dim(all.res.motifs.disease.process)[1] > 0)
  exportNetworkProperties (all.res.motifs.disease.process,file.path(output.path,"disease.process.4node"),disease,pval.cutoff,all.res.disease.process.tissue.specific)
}
...
ExportMotifs =function(net.path,output.path,evidence,species)
{
  ...
  if(dim(tfmir.pairs)[1] >0) 
  {
    ...
    A = net.path
    A=strsplit(A,"/")
    A[[1]][length(A[[1]])-1]=paste(A[[1]][length(A[[1]])-1],".4node",sep="")
    B=c()
    B=A[[1]][1]
    ...
    #print(c(dim(tfmir.pairs.tf.mirna)[1],dim(tfmir.pairs.tf)[1],dim(tfmir.pairs.mirna)[1],dim(tfmir.pairs.gene.gene)[1]))
    #print(c(dim(tfmir.pairs.tf.mirna)[1],dim(tfmir.pairs.tf)[1],dim(tfmir.pairs.mirna)[1],dim(tfmir.pairs.gene.gene)[1]))
