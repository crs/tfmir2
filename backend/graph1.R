...
getPutativeTFmiRPairs4Node_tf_mirna=function (net)
{
  ...
  for(i in 1 : length(tfs))
  {
    for(j in 1 :length(mirnas))
    {
      ...
      #print(c("targets tf and mirna",targets.tf,targets.mirna))
      ...
      if(length(regulators) > 0 && regulators != ""){
        ...
        for( z in 1:length(regulators[[1]]))  
        {
          ...
          no = no + 1
          #print(c(i,j))
          #print(targets.tf.mirna)
        }
      }
    }
  }
  #print(c("4Node_tf_mirna",no))
  return(tfmir.pairs)
}
  ...
  for(i in 1 : length(tfs))
  {
    for(j in 1 :length(mirnas))
    {
      ...
      if((length(grep(", ",regulators))>0) && regulators != "" && (length(grep(", ",targets))>0) && targets != ""){
        ...
        #print(c("length,r,t,tf",regulators,length(regulators),targets,length(targets)))
      }
      if(length(regulators) > 0 && regulators != ""){
        #print(c(length(regulators),regulators))
  ...
  for(i in 1 : length(tfs))
  {
    for(j in 1 :length(mirnas))
    {
      ...
      if((length(grep(", ",regulators))>0) && regulators != "" && (length(grep(", ",targets))>0) && targets != ""){
        ...
        regulators = c(unlist(regulators1),unlist(regulators2))
        targets = c(unlist(targets1),unlist(targets2))
        #print(c("length,r,t,empty",regulators,length(regulators),targets,length(targets)))
      }
      if(length(regulators) > 0 && regulators != ""){
        for( z in 1:length(regulators))  
        {
...
{       
    if(dim(tfmir.pairs)[1] != 0){
      ...
      tfmir.pairs.new
      #if(nrow(tfmir.pairs.new) > 0 )
      #write.table(tfmir.pairs.new, "/Users/Maryam/Desktop/mydata.txt", sep="\t")
    }
    else{
      tfmir.pairs
      #print("2nd entered")
  ...
  if(length(tfmir.pairs.FFL) > 0){
    ...
    {  
      ...
      for( i in 1 : as.integer(config$NO_OF_Times_FOR_Network_Randomizations_4Node))
      {
        ...
        #tfmir.pairs.random.FFL = list(tfmir.pairs.random.tf.mirna)
        #print(c(i,dim(tfmir.pairs.random.tf.mirna)[1]))
        for(z in 1:no.types)
        {
          #begin tf-miRNA
      ...
      ########################################################################
      print(length(motifs.FFL.primary))
      motifs.FFL = c()
